...
// Generate Fan-shaped Card Layout
function generateCards() {
  ...
  const totalAngle = 100; // total spread angle
  const startAngle = -totalAngle / 2;
  const angleStep = totalAngle / (CARD_COUNT - 1);

  selected.forEach((card, index) => {
    const angle = startAngle + angleStep * index;
    const cardEl = document.createElement('div');
    cardEl.className = 'tarot-card';
    cardEl.style.transform = `translateX(-50%) rotate(${angle}deg)`;
    ...
    cardEl.innerHTML = `
      <div class="tarot-card__inner">
        ...
        <div class="tarot-card__front">
          ...
          <span class="tarot-card__front-name">${card.name}</span>
        </div>
      </div>
    `;
    cardEl.addEventListener('click', () => selectCard(cardEl, card, angle));

    // Staggered entrance
    cardEl.style.opacity = '0';
    setTimeout(() => {
      cardEl.style.transition = 'opacity 0.4s ease, transform 0.4s ease, filter 0.3s ease';
...
// Select Card
function selectCard(cardEl, card, angle) {
  if (selectedCard) return;
  selectedCard = card;

  // Flip selected card and bring to center
  cardEl.classList.add('tarot-card--flipped');
  cardEl.style.transform = 'translateX(-50%) rotate(0deg) translateY(-20px) scale(1.3)';
  ...
  setTimeout(() => showResult(card), 1500);
}
...
function showResult(card) {
  ...
  document.getElementById('result-name-en').textContent = card.nameEn;
...
function shareTwitter() {
  if (!selectedCard) return;
  const text = encodeURIComponent(`x-TENのタロット占いで「${selectedCard.name}」が出ました。\n${selectedCard.message.slice(0, 60)}...\n`);
